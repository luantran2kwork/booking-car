...
export default class GlobalCache {
  ...
  static GetSystemParamNameByID(id: number): string | null {
    let item = GlobalCache.SystemParameters.find(
      (en) => en.SystemParameterID === id
    );
    if (item) {
      return item.Name;
    }
  ...
  static GetSystemParamsByFeatureID(
    featureID: number
  ): Array<iSystemParameter> {
    return GlobalCache.SystemParameters.filter(
      (en) => en.FeatureID === featureID
    );
  }

  // Lấy danh sách SystemParam by featureID và cột mở rộng Ext1i
  static GetSystemParamsByFeatureAndExt1i(
    featureID: number,
    ext1i: number
  ): Array<iSystemParameter> {
    return GlobalCache.SystemParameters.filter(
      (en) => en.FeatureID === featureID && en.Ext1i === ext1i
    );
  }
