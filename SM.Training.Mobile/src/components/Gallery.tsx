...
import MobileLog from "../Entities/MobileLog";
...
import MobileLogDto from "../DtoParams/MobileLogDto";
...
export default class Gallery extends React.Component<iProps, iState> {
  ...
  async handleRemove(attachment: adm_Attachment, id) {
    try {
      var req = new AttachmentDto();
  ...
  _pickImage = async () => {
    try {
      ...
      var reqLog = new MobileLogDto();
      var log = new MobileLog();
      log.ContentLog = JSON.stringify(result);
      log.RefType = this.state.SelectedDefault.RefType;
      log.DeviceID = Device.deviceName;
      log.RefID = this.state.SelectedDefault.RefID;
      reqLog.MobileLog = log;

      let resLog = await HttpUtils.post<MobileLogDto>(
        ApiUrl.MobileLog_Execute,
        SMX.ApiActionCode.InsertLog,
  ...
  _handleImagePicked = async (result) => {
    try {
      this.props.GlobalStore.ShowLoading();
      if (result.cancelled) this.props.GlobalStore.HideLoading();
      var reqLog = new MobileLogDto();
      var log = new MobileLog();
      log.ContentLog = result.base64;
      log.RefType = this.state.SelectedDefault.RefType;
      log.DeviceID = Device.deviceName;
      log.RefID = this.state.SelectedDefault.RefID;
      reqLog.MobileLog = log;
  ...
  _handleImagePickedSDVT = async (a, result) => {
    try {
      ...
      this.props.Images.forEach((element) => {
        (att.RefID = element.RefID), (att.RefType = 1);
      });
            ...
            >
              ...
              {this.props.allowRemove != null && this.props.allowRemove ? (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    //console.log("item",this.state.SelectedFullScreen);
                    this.handleRemove(
                      this.state.SelectedFullScreen,
                      this.state.SelectedFullScreen.AttachmentID
                    );
                  }}
                >
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
